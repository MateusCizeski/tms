...
export function Drivers() {
  ...
  async function handleSubmit(data: Omit<Driver, 'id' | 'created_at' | 'updated_at' | 'is_active'>) {
    if (editTarget) {
  ...
  async function handleToggle() {
    ...
    toast.success(`Motorista ${toggleTarget.is_active ? 'inativado' : 'ativado'}!`);
      ...
      <div className="bg-white rounded-xl shadow overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-600">
            <tr>
              {['Nome', 'CPF', 'Nº CNH', 'Categoria', 'Telefone', 'Status', 'Ações'].map((h) => (
            ...
            {drivers.map((d) => (
              <tr key={d.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <span className="bg-gray-100 px-2 py-0.5 rounded font-mono text-xs">{d.cnh_category}</span>
                </td>
